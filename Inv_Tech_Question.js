...
//
//INSTRUCTIONS:
//FIRST, change the PATH of the "newfile" const on line 14 to the path where you have latinmap.js stored on your computer.
//
...
const newfile = require("/Users/LiamOBrien/Desktop/invisible/Invisible_Technical_Question/latinmap.js");//include map that changes undifined letters like 'ã' to readable letters like 'a'.
...
function time_and_weather(cityorzip){
  let output = ``;
  let apiKey = 'c52d902070cf78c08d87733170517dca';
  let darkskyapikey = '7a664d504e7b11c2da6e8fb7dd9081c5';
  let country = 0;
  let iszip = 0;//bool
  let cleancityorzip = cityorzip.latinise(); //clean the input to make sure it doesn't use any undefined symbols.
  //Use openweathermap api to access latitude and longitude of inputed location. 
  let url = '';
  if(isNaN(cityorzip)){
    iszip = 0;
    try {
      ...
      console.log(`The city, ${cityorzip}, doesn't exist`);
      return;
    }
  } else {
    iszip = 1;
    try {
      ...
      console.log(`postal code, ${cityorzip}, doesn't exist`);
      return;
    }
  }
  //access open weather map api.
    ...
    if(err){
      ...
      DSrequest(DS_url, function (DSerr, DSresponse, DSbody) {
        if(DSerr){
          ...
          //log the time and weather to the console. If inputted data is bad (i.e. postal code or city doesn't exist), code will print error message and then continue reading next input. 
          try {
            if(iszip == 1){
            ...
            console.log(`${cityorzip} is an undfefined input.`);// error message if city or postal code doesn't exist.
            return;
          }
        }
      });
